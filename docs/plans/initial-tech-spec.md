...
- Rate-limit poll creation and voting, plus per-creator / per-poll size caps (Phase 10).
...
3. **Guest vote identity.** Guests are identified only by display name (the `pollId_guestName` unique constraint), so two guests sharing a name collide on one `Participant` and the later submit silently overwrites the earlier one's ballot; anyone can likewise overwrite a named voter's votes by reusing their name. This is a **guest-voting write-time collision, independent of the anonymity flag** (which only governs Phase 7 results *rendering*) — anonymity merely makes the overwrite undetectable; visible voting makes it noticeable. Signed-in voters (keyed on `userId`) are unaffected. Acceptable for MVP guest UX; revisit before any high-stakes use. Candidate mitigations, cheapest first:
   - **Guest cookie token (recommended).** On first guest cast, mint a random token, store it in an httpOnly cookie and on a new nullable `Participant.guestToken` column; key guest identity on `pollId_guestToken` instead of `pollId_guestName`. A resubmit counts as the same guest only when the token matches; a different browser using the same name becomes a distinct participant (allow duplicate display names with the token as the real key). Stops cross-person overwrite and impersonation, keeps the no-login UX, and survives the inline-sign-in OAuth round-trip if set beforehand. Cost: a schema migration + constraint rework; a lost cookie means a guest can't edit their own vote (acceptable — guests already can't reliably return). **Deferred** — likely its own small phase or folded into Phase 7/8; this open-question note is the interim acknowledgement.
