...
**Phases 1–10 are done.** A Next.js App Router + TypeScript app is live at the repo root with Prisma against Postgres and Auth.js v5 Google sign-in (Prisma adapter + **database sessions** + full access control — see below). The portable production image is proven by the `Dockerfile` (`docker build`); `docker compose` is dev-only and runs just a local Postgres. **Phase 2** plumbed the full design-token set from `docs/design/colors_and_type.css` into Tailwind `theme.extend` + CSS variables (`app/globals.css`), wired self-hosted fonts (Space Grotesk via `next/font/local`, Inter via `next/font/google`) and `lucide-react`, and shipped the `data-theme` light/dark mechanism (no-flash inline script + `lib/theme.ts` + `localStorage` `herd-theme`). Token specimen page: `/dev/tokens`. **Phase 3** shipped the presentational component kit (`components/ui/`, showcase at `/dev/gallery`). **Phase 4** replaced the placeholder with the full Prisma data model and the access-control system — see "Auth & access control" below. **Phase 5** shipped the creator-gated create + share flow (`/create` → `CreateForm` client + `actions.ts` server action; multi-select calendar, sticky 30-min time range, per-poll anonymity toggle default-visible, slug + UTC `TimeOption` rows) and the resolving `/p/{slug}` page. It added `lib/time.ts` (the 5-zone ET/CT/MT/PT/GMT enum ↔ IANA + DST-correct wall-time↔UTC conversion via `date-fns-tz`) and promoted the audit action names to a single source of truth (`AUDIT_ACTIONS` / `AUDIT_ACTION_VALUES` in `lib/access.ts`) that both the `logAction` writers and the `/admin` filter derive from. **Phase 6** turned `/p/{slug}` into the live vote screen (`app/p/[slug]/page.tsx` server component → `VoteForm` client + `actions.ts`): per-slot 3-way `Segmented` (tap-to-clear), a guest display-name input with an inline Google sign-in affordance, and a sticky progress/submit bar that collapses to a "Saved" toast. Persistence lives in `lib/votes.ts` (`saveBallot`/`loadBallot` + the UI `yes/maybe/no` ↔ Prisma `Response` `yes/ifneedbe/no` mapping): a marked slot upserts an `Availability` row, an unmarked one deletes it, and resubmitting reuses the same `Participant` (logged-in voters key on `userId`; guests keyed on `guestName` until Phase 9 rekeyed them — see below). In-progress votes survive the inline-sign-in OAuth round-trip via a per-slug `localStorage` draft restored on return; submits write `vote.cast`/`vote.update` audit rows. **Phase 7** shipped the results screen (`app/p/[slug]/results/page.tsx` server component → `ResultsView` client) and host-only finalize (`app/p/[slug]/finalize/actions.ts`). All scoring + the anonymity gate live in `lib/results.ts` (`summarizeResults`, the pure best-fit core: `score = yes*3 + maybe - no*4`, top-score ties highlighted equally, "works for everyone" = zero hard-No with ≥1 available, the lead slot, and the shared `pollResultsInclude`). **Anonymity is enforced in the data layer** — when `anonymousVoting`, `attendees` is dropped to `null` before data leaves `lib/results.ts`, so identity can't leak through any caller (and results are **never** sourced from `AuditLog`). Finalizing writes `Poll.finalTimeOptionId` (host = creator or owner, re-checked each call), logs `poll.finalize`, and **ends voting** — a finalized poll is treated as closed in both the vote page and `submitVote`. `finalized` is a *derived* status (`finalTimeOptionId` set), not a `PollStatus` enum value. **Phase 8** replaced the scaffold `app/page.tsx` with the creator home ("My polls"): a server component (auth-gated → `/signin`; picks the list / empty / non-creator variant) feeding the `CreatorHome` client. `lib/polls.ts` `listPollsForCreator(userId)` shapes the rows (newest first, status pill, date span via `lib/time.ts` `formatSpan`, distinct-responder count, and the leading/finalized slot via `summarizeResults`); `components/ui/StatusPill.tsx` renders open/closed/finalized. The create-flow "View responses" button now lands on `/p/{slug}/results`. **Phase 9** made guest identity persistent per-browser: guests now key on an opaque client-minted `guestKey` (`@@unique([pollId, guestKey])`; `guestName` demoted to a display label — same-name guests no longer collide, and a rename updates the same row). The key + last-used name live in `localStorage` (`herd-guest-id` / `herd-guest-name`, global across polls — `lib/guest.ts` holds the storage keys, `mintGuestKey`, and the server-side `isValidGuestKey` shape check); they're minted/persisted on first successful submit, and a returning guest's saved ballot is hydrated client-side on mount via the `loadGuestBallot` server action (`lib/votes.ts` `loadGuestRecord`) with precedence draft > saved ballot > empty, the submit button flipping to "Update availability", plus a quiet "Not you? Start fresh" link that clears the stored identity (shared devices). The `guestKey` is a per-browser credential: sent only with its owner's own submit/load, never returned to any other viewer, never written to `AuditLog`. **Phase 10** hardened the deploy: the `Dockerfile` now ships a **non-root** slim runtime (default target) plus a separate **`migrate` target** — the one-shot `prisma migrate deploy` image — and CI (`.github/workflows/publish-image.yml`) publishes both to `ghcr.io/matthewshan/herd-scheduler` on merge/release (`-migrate` tag suffix for the migration image; on the k3s target it runs as a sync-wave-gated ArgoCD Job ahead of the app's wave). Abuse guards landed portable and env-tunable: `lib/rate-limit.ts` (in-process fixed-window — per-creator poll creation, per-IP voting; **per replica** by design) and row-count caps in `lib/limits.ts` (`maxPollsPerCreator`/`maxSlotsPerPoll`/`maxParticipantsPerPoll`; `saveBallot` enforces the participant cap transactionally via `ParticipantLimitError` and never blocks an existing voter editing their own ballot). Operational runbooks for the three reference deployments live in `docs/deployment.md`; the cluster manifests are owner-managed in the `k3s-homelab` repo (`applications/herd-scheduler/`). Phase 2 features are captured as **backlog briefs** in `docs/plans/phases/phase-2-backlog/` (timezone toggle, poll expiry, email notifications — provider still open, spec §11 Q1); they are documented, not built. Phase 11 (testing) remains — see `docs/plans/phases/`.
...
>
> **Whenever you create a PR, invoke the `visual-capture` skill first** to capture/refresh the relevant flow GIF(s), then embed them in the PR description. Do this for every PR that touches a user-facing screen or flow — don't open the PR until the GIF is captured and committed.
...
- **TypeScript conventions:** follow `docs/typescript-standards.md` — notably, component props are declared as named `interface`s (never inline object-type literals), `type` is reserved for unions/aliases, and native-element wrappers extend the matching `*HTMLAttributes` type. **State management is tiered:** `useState` for local/ephemeral UI (independent flags are fine — don't merge them to cut the count), `useReducer` for a local state machine, and a Zustand store *only* for cross-component or complex form/domain state (e.g. `app/create/store.ts`); never park purely-local UI state in a global store.
- **Front-end work ships with a GIF.** Every PR that adds or changes a user-facing screen/flow must include a captured GIF of it (committed under `docs/screenshots/phase-<n>/`, embedded in the PR description) via the `pnpm capture:visual` pipeline — see the "Visual capture" note above and `docs/context-engineering/visual-capture.md`. **Anytime you create a PR, invoke the `visual-capture` skill before opening it** to capture/refresh the GIF(s) for the affected flow.
